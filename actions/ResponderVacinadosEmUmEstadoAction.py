...
class ResponderVacinadosEmUmEstadoAction(Action):
    def __init__(self) -> None:
        ...
        self.caminho_arquivo_csv = path.join(
            diretorio, 'cases-brazil-states.csv')
        return
    ...
    def run(
        ...
        dataframe_estado = dataframe.query(f"state == '{uf}'")[
            ['date', 'vaccinated', 'vaccinated_second']]
        ...
        data_mais_recente = dataframe_estado_mais_recente.iloc[0]['date'].strftime(
            '%d/%m/%Y')
